(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();function e(){let e=document.getElementById(`bubbles-canvas`);if(!e)return;let t=e.getContext(`2d`);if(!t)return;let n=window.innerWidth,r=window.innerHeight;e.width=n,e.height=r;let i=[],a=window.innerWidth<768?10:18,o=.11,s=.018;function c(e,t){return{x:e??Math.random()*n,y:t??Math.random()*r,radius:Math.random()*90+40,vx:(Math.random()-.5)*.18,vy:(Math.random()-.5)*.09,alpha:Math.random()*(o-s)+s,alphaSpeed:(Math.random()*.003+.001)*(Math.random()<.5?1:-1),hue:Math.random()*30-10}}function l(){i=[];for(let e=0;e<a;e++)i.push(c())}l();let u,d=!0;function f(){if(!(!t||!d)){t.clearRect(0,0,n,r);for(let e of i){e.x+=e.vx,e.y+=e.vy,e.x<-e.radius?e.x=n+e.radius:e.x>n+e.radius&&(e.x=-e.radius),e.y<-e.radius?e.y=r+e.radius:e.y>r+e.radius&&(e.y=-e.radius),e.alpha+=e.alphaSpeed,e.alpha>=o?(e.alpha=o,e.alphaSpeed=-Math.abs(e.alphaSpeed)):e.alpha<=s&&(e.alpha=s,e.alphaSpeed=Math.abs(e.alphaSpeed));let i=340+e.hue,a=t.createRadialGradient(e.x,e.y,e.radius*.15,e.x,e.y,e.radius);a.addColorStop(0,`hsla(${i}, 100%, 60%, ${e.alpha*.65})`),a.addColorStop(.5,`hsla(${i}, 100%, 55%, ${e.alpha*.3})`),a.addColorStop(1,`hsla(${i}, 100%, 50%, 0)`),t.beginPath(),t.arc(e.x,e.y,e.radius,0,Math.PI*2),t.fillStyle=a,t.fill()}u=requestAnimationFrame(f)}}let p;function m(){clearTimeout(p),p=setTimeout(()=>{n=window.innerWidth,r=window.innerHeight,e.width=n,e.height=r,l()},150)}function h(){d=!document.hidden,d?u=requestAnimationFrame(f):cancelAnimationFrame(u)}return f(),window.addEventListener(`resize`,m,{passive:!0}),document.addEventListener(`visibilitychange`,h),()=>{window.removeEventListener(`resize`,m),document.removeEventListener(`visibilitychange`,h),cancelAnimationFrame(u)}}var t=[{name:`Управление сообществами`,icon:`users`,category:`Навыки`},{name:`Программирование на Java`,icon:`code`,category:`Навыки`},{name:`AI Промпт-инженеринг`,icon:`brain`,category:`Навыки`},{name:`Контент-Менеджмент`,icon:`megaphone`,category:`Навыки`},{name:`Брендинг сообществ`,icon:`palette`,category:`Навыки`},{name:`Видеомонтаж`,icon:`monitor`,category:`Навыки`},{name:`Дизайн`,icon:`pencil`,category:`Навыки`}],n=[{name:`Figma`,icon:`figma-logo`},{name:`IntelliJ IDEA`,icon:`code`},{name:`Adobe Photoshop`,icon:`image-square`},{name:`Adobe After Effects`,icon:`film-strip`},{name:`Adobe Premiere`,icon:`video`},{name:`Adobe Audition`,icon:`waveform`},{name:`Capcut`,icon:`scissors`},{name:`Excel / Google docs`,icon:`table`},{name:`Grok / DeepSeek / ChatGPT`,icon:`brain`}],r=[{id:1,title:`Milkshake`,slug:`milkshake`,subtitle:`Discord`,image:`./images/milkshake/cover.webp`,category:[`discord`,`code`],starred:!0,shortDesc:`Сообщество для знакомств и совместных игр`,fullContent:`
      <div class="space-y-6">
...
dependencies {
  ...
  `}];function i(){setTimeout(()=>{window.PhosphorIcons&&typeof window.PhosphorIcons.loadIcons==`function`&&window.PhosphorIcons.loadIcons()},10)}function a(e){requestAnimationFrame(()=>{requestAnimationFrame(()=>{e.querySelectorAll(`.opacity-0`).forEach(e=>{e.classList.add(`animate-fade-in-up`)})})})}function o(){let e=document.getElementById(`skills`);e&&(e.innerHTML=`
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
      ${t.map((e,t)=>`
        <div class="skill-card opacity-0"
             style="animation-delay: ${t*55}ms">
          <i class="ph-fill ph-${e.icon} text-3xl text-[#FF337D] flex-shrink-0"></i>
          <span class="font-medium text-gray-200 text-sm md:text-base">${e.name}</span>
        </div>
      `).join(``)}
    </div>
  `,i(),a(e))}function s(){let e=document.getElementById(`skills`);e&&(e.innerHTML=`
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
      ${n.map((e,t)=>`
        <div class="bg-[#1a1a1a] rounded-2xl p-5 flex items-center gap-4 hover:bg-[#1e1e1e] transition-all duration-300 border border-white/5 hover:border-[#FF337D]/40 group opacity-0"
             style="animation-delay: ${t*55}ms">
          <i class="ph-fill ph-${e.icon} text-3xl text-[#FF337D] flex-shrink-0 transition-transform duration-300 group-hover:scale-110"></i>
          <span class="font-medium text-gray-200">${e.name}</span>
        </div>
      `).join(``)}
    </div>
  `,i(),a(e))}function c(e){return e.map((e,t)=>`
    <div onclick="openProject(${e.id})"
         ...
         style="animation-delay: ${t*45}ms; will-change: transform;">
      ${e.starred?`
  ...
  `).join(``)}function l(e){let t=document.getElementById(`projects-grid`);if(!t)return;let n=u(e),r=()=>{if(n.length===0){t.innerHTML=`<div class="col-span-full text-center text-gray-400 py-12 opacity-0 animate-fade-in-up">Проектов пока нет</div>`;return}t.innerHTML=c(n),i(),a(t)},o=Array.from(t.children);o.length>0?(o.forEach(e=>{e.style.transition=`opacity 0.08s ease, transform 0.08s ease`,e.style.opacity=`0`,e.style.transform=`scale(0.97) translateY(-3px)`}),setTimeout(r,100)):r()}function u(e){return[...e].sort((e,t)=>e.starred&&!t.starred?-1:!e.starred&&t.starred?1:e.id-t.id)}function d(e){document.querySelectorAll(`.filter-btn`).forEach(t=>{let n=t.getAttribute(`data-filter`)===e;t.classList.toggle(`active`,n),t.setAttribute(`aria-pressed`,n?`true`:`false`)});let t;t=e===`all`?r:e===`starred`?r.filter(e=>e.starred===!0):r.filter(t=>t.category.includes(e)),l(t)}function f(e){document.querySelectorAll(`.tab-btn`).forEach((t,n)=>{t.classList.toggle(`active`,n===e),t.setAttribute(`aria-selected`,n===e?`true`:`false`)}),e===0?o():s()}function p(){document.querySelectorAll(`#modal .carousel`).forEach(e=>{if(e.hasAttribute(`data-initialized`))return;e.setAttribute(`data-initialized`,`true`);let t=e.querySelector(`.carousel-slides`),n=e.querySelector(`.carousel-prev`),r=e.querySelector(`.carousel-next`),i=e.querySelector(`.carousel-dots`);if(!t||!n||!r||!i)return;let a=0,o=t.children.length;if(o<=1){n.style.display=`none`,r.style.display=`none`;return}i.innerHTML=``;for(let e=0;e<o;e++){let t=document.createElement(`div`);t.classList.add(`dot`),e===0&&t.classList.add(`active`),t.addEventListener(`click`,()=>l(e)),i.appendChild(t)}let s=i.querySelectorAll(`.dot`);function c(){t.style.transform=`translateX(-${a*100}%)`,s.forEach((e,t)=>{e.classList.toggle(`active`,t===a)})}function l(e){a=(e+o)%o,c()}n.addEventListener(`click`,()=>l(a-1)),r.addEventListener(`click`,()=>l(a+1)),c()})}function m(e){let t=r.find(t=>t.id===e);if(!t)return;let n=document.getElementById(`modal`),i=document.getElementById(`modal-content`);i.innerHTML=(t.image?`
      ...
      </div>`:``)+t.fullContent,n.classList.remove(`hidden`,`modal-closing`),n.classList.add(`flex`),document.body.style.overflow=`hidden`,setTimeout(()=>{p()},100)}function h(){let e=document.getElementById(`modal`);!e||e.classList.contains(`hidden`)||(e.classList.add(`modal-closing`),setTimeout(()=>{e.classList.add(`hidden`),e.classList.remove(`flex`,`modal-closing`),document.body.style.overflow=``;let t=window.location.pathname;t!==`/`&&t!==``&&window.history.pushState({},``,`/`)},220))}function g(){document.body.insertAdjacentHTML(`beforeend`,`
